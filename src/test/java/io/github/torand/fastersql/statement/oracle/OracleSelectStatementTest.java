...
class OracleSelectStatementTest extends OracleTest {
    ...
    void shouldHandleScalarMathFunctions() {
        PreparableStatement stmt =
            select(PRODUCT.NAME, round(PRODUCT.PRICE).as("ROUND"), abs(-1.0).as("ABS"), ceil(PRODUCT.PRICE).as("CEIL"), floor(PRODUCT.PRICE).as("FLOOR"), ln(Math.E).as("LN"), exp(1.0).as("EXP"), sqrt(4.0).as("SQRT"), pow($(3), 2.0).as("POW"))
                .from(PRODUCT)
                .orderBy(PRODUCT.NAME.asc());
        ...
        statementTester()
            ...
            .assertParams(-1.0, Math.E, 1.0, 4.0, 3, 2.0)
            .assertRowCount(5)
            .assertRow(2,
    ...
    void shouldHandleCastFunction() {
        PreparableStatement stmt =
            select(PRODUCT.NAME, PRODUCT.PRICE.plus(cast(1).as(decimal(2,1))).as("DEC_CAST"), cast("2").as(varchar(1)).as("STR_CAST"))
                .from(PRODUCT)
                .orderBy(PRODUCT.NAME.asc());

        statementTester()
            .assertSql("""
                select PR.NAME PR_NAME, PR.PRICE + cast(? as number(2, 1)) DEC_CAST, cast(? as varchar2(1)) STR_CAST \
                from PRODUCT PR \
                order by PR.NAME asc"""
            )
            .assertParams(1, "2")
