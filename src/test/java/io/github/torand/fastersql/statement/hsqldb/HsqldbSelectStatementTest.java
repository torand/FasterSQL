...
public class HsqldbSelectStatementTest extends HsqldbTest {
    ...
    public void shouldHandleScalarMathFunctions() {
        ...
        statementTester()
            .assertSql("""
                select PR.NAME PR_NAME, round(PR.PRICE) ROUND, abs(?) ABS, ceil(PR.PRICE) CEIL, floor(PR.PRICE) FLOOR, ln(?) LN, exp(?) EXP, sqrt(?) SQRT, power(cast(? as decimal), cast(? as decimal)) POW \
                from PRODUCT PR \
