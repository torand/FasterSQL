...
public class HsqldbSelectStatementTest extends HsqldbTest {
    ...
    public void shouldHandleScalarMathFunctions() {
        ...
        statementTester()
            .assertSql("""
                select PR.NAME PR_NAME, round(PR.PRICE) ROUND, abs(?) ABS, ceil(PR.PRICE) CEIL, floor(PR.PRICE) FLOOR, ln(?) LN, exp(?) EXP, sqrt(?) SQRT, power(cast(? as decimal), ?) POW \
                from PRODUCT PR \
                order by PR.NAME asc"""
            )
    ...
    public void shouldHandleSimpleCaseExpressions() {
        PreparableStatement stmt =
            select(
                case_(CUSTOMER.LAST_NAME)
                    .when(cast("Nordmann").as(varchar(30))).then("Norwegian")
                    .when(cast("Hansen").as(varchar(30))).then("Dane")
                    .when(cast("Svensson").as(varchar(30))).then("Swede")
                    .else_(cast("Not Scandinavian").as(varchar(30)))
                .end().as("Nationality"))
                .from(CUSTOMER)
                .orderBy(colPos(1).asc());
        ...
        statementTester()
            .assertSql("""
                ...
                when cast(? as varchar(30)) then ? \
                when cast(? as varchar(30)) then ? \
                when cast(? as varchar(30)) then ? \
                else cast(? as varchar(30)) \
                end Nationality \
                from CUSTOMER C \
                order by 1 asc"""
            )
