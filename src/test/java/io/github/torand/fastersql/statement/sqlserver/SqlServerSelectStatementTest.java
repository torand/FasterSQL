...
class SqlServerSelectStatementTest extends SqlServerTest {
    ...
    void shouldHandleExpressionsInProjection() {
        ...
        statementTester()
            ...
            .assertRow(2,
                "C_LAST_NAME", is("NORDMANN"),
                "C_FIRST_NAME", is("ola"),
                "TOTAL", isBigDecimalCloseTo(5433.5, 0.001),
                "C_ZIP_CODE", isNull(),
                "PI", isDouble(3.14)
            )
            .verify(stmt);
    ...
    void shouldHandleExpressionsInPredicate() {
        ...
        statementTester()
            .assertSql("""
                ...
                inner join PURCHASE_ITEM PI on PU.ID = PI.PURCHASE_ID \
                inner join PRODUCT PR on PI.PRODUCT_ID = PR.ID \
                where PR.PRICE > ? * (? + cast(? as numeric(2,0))) \
    ...
    void shouldHandleAggregatesWithGroupingAndOrdering() {
        ...
        statementTester()
            ...
            .assertRow(1,
                ...
                "PURCHASED_VALUE", isBigDecimalCloseTo(5433.5, 0.001)
            )
            .assertRow(2,
    ...
    void shouldHandleFilteredGroups() {
        PreparableStatement stmt =
            select(PRODUCT.NAME, sum(PRODUCT.PRICE.times(PURCHASE_ITEM.QUANTITY)).as("PURCHASED_VALUE"), max(PURCHASE_ITEM.QUANTITY).as("MAX_QNTY"))
                ...
                .join(PRODUCT.ID.on(PURCHASE_ITEM.PRODUCT_ID))
                .groupBy(PRODUCT.NAME)
                .having(/*alias("PURCHASED_VALUE")*/sum(PRODUCT.PRICE.times(PURCHASE_ITEM.QUANTITY)).gt(1).and(max(PURCHASE_ITEM.QUANTITY).lt(100)))
                .orderBy(alias("PURCHASED_VALUE").asc());
    ...
    void shouldHandleSystemFunctions() {
        ...
        statementTester()
            .assertSql("""
                select C.ID C_ID, current_timestamp CTS, getdate() CD \
                from CUSTOMER C"""
            )
            .assertRowCount(3)
            .verify(stmt);
    ...
    void shouldHandleScalarMathFunctions() {
        ...
        statementTester()
            .assertSql("""
                select PR.NAME PR_NAME, round(PR.PRICE, 0) ROUND, abs(?) ABS, ceiling(PR.PRICE) CEIL, floor(PR.PRICE) FLOOR, log(?) LN, exp(?) EXP, sqrt(?) SQRT, power(?, ?) POW \
                from PRODUCT PR \
                order by PR.NAME asc"""
            )
            ...
            .assertRow(2,
                ...
                "ABS", isInteger(1),
                "CEIL", isBigDecimalCloseTo(5434.0, 0.1),
                "FLOOR", isBigDecimalCloseTo(5433.0, 0.1),
                "LN", isDouble(1.0),
    ...
    void shouldHandleSetOperations() {
        // SQL Server does not support ORDER BY in set operation operands and no parentheses around operands
